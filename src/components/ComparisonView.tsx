...
export function ComparisonView({ onBack }: ComparisonViewProps) {
  ...
  const filteredEngines = Object.values(ENGINE_CONFIGS).filter(
    engine => filterProvider === 'all' || engine.provider === filterProvider
  )
                  ...
                  {filteredEngines.map((engine) => (
                    <div
                      ...
                      className={`flex items-center space-x-3 p-2 rounded hover:bg-secondary/10 cursor-pointer ${
                        selectedEngines.has(engine.id) ? 'bg-primary/10' : ''
                      }`}
