                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                      {Object.entries(ENGINE_CONFIGS).map(([key, engine]) => (
                        <div
                          key={key}
                          className={`flex items-start gap-3 p-3 rounded-lg border cursor-pointer transition-all ${
                            selectedEngines.has(key as AIEngine)
                              ? 'bg-primary/10 border-primary/50'
                              : 'bg-background/50 border-border/30 hover:border-border/50'
                          }`}
                          onClick={() => toggleEngine(key as AIEngine)}
                        >
                          <Checkbox
                            checked={selectedEngines.has(key as AIEngine)}
                            className="mt-0.5"
                          />
                          <div className="flex-1 min-w-0">
