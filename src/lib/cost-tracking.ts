...
export const MODEL_COSTS: Record<AIEngine, { input: number; output: number }> =
  {
    ...
    'groq-llama-3.3-70b': { input: 0.0, output: 0.0 },
    'groq-llama-3.1-8b': { input: 0.0, output: 0.0 },
    ...
    'huggingface-meta-llama-3.3-70b': { input: 0.0, output: 0.0 },
    'huggingface-qwen-2.5-72b': { input: 0.0, output: 0.0 },
  }
...
export function calculateCost(
  ...
  if (!costs) return 0 // Unpriced runs remain counted; provider billing is authoritative.
...
export function logCost(
  ...
  try {
    ...
    /* Cost estimates must not discard a completed response. */
  }
...
export function getUsageStats(): UsageStats {
  ...
  entries.forEach((entry) => {
    ...
    stats.costByMode[entry.mode] =
      (stats.costByMode[entry.mode] || 0) + entry.cost
  })
