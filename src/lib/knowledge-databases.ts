...
export async function queryWolframAlpha(query: string): Promise<string> {
  try {
    const response = await apiFetch('/api/knowledge/wolfram', {
      method: 'POST',
...
export async function searchWeb(
  ...
  try {
    const response = await apiFetch('/api/knowledge/web', {
      method: 'POST',
...
export async function searchArxiv(
  ...
  try {
    const response = await apiFetch('/api/knowledge/arxiv', {
      method: 'POST',
      body: JSON.stringify({ query, limit }),
    })
...
export async function aggregateKnowledge(
  query: string,
  sources: Array<
    'wikipedia' | 'wolfram' | 'web-search' | 'arxiv' | 'pubmed'
