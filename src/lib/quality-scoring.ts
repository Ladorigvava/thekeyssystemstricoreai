...
export function getAverageScoresByEngine(): Record<string, {
  ...
  const byEngine: Record<string, QualityScore[]> = {}
  
  responses.forEach(r => {
    if (!byEngine[r.engine]) {
      byEngine[r.engine] = []
    }
    byEngine[r.engine].push(r.score)
  })
  
  const averages: Record<string, any> = {}
  
  Object.keys(byEngine).forEach(engine => {
    const scores = byEngine[engine]
    const count = scores.length
    
    averages[engine] = {
      count,
      avgOverall: scores.reduce((sum, s) => sum + s.overall, 0) / count,
      avgClarity: scores.reduce((sum, s) => sum + s.clarity, 0) / count,
      avgAccuracy: scores.reduce((sum, s) => sum + s.accuracy, 0) / count,
      avgUsefulness: scores.reduce((sum, s) => sum + s.usefulness, 0) / count,
      avgDepth: scores.reduce((sum, s) => sum + s.depth, 0) / count,
      avgCreativity: scores.reduce((sum, s) => sum + s.creativity, 0) / count
    }
  })
